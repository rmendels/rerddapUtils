...
```
 ...
 The [R]{style="color:red"} package `rerddapUtils` is a set of four main functions designed to extend the `rerddap` package to provide capabilities requested by users, and that meet specialized needs which are better dealt with in a separate package.  The package also provides two helper functions that estimate the size of a proposed `rerddap::griddap()` request.
...
splits a very large request into pieces defined by 'request_split',  and then aggregates the parts either in memory,  in a duckdb database, in a netcdf4 file, or in a partitioned parquet file.  This allows for requests larger than 2GB to be made in an appropriate manner.
 
Note that `griddap_season()` and `griddap_split()` are designed to mostly have the same interface as `rerddap::griddap()`,  except for one or two  additional function arguments.  Also some arguments are interpreted differently, such as in both functions the argument 'store = disk()' is ignored.  In `griddap_season()` the 'fmt' argument is ignored  while 'season' defines the days during the year to make the extract.

In  `griddap_split()` the argument 'fmt' defines whether the aggregate download should be stored in memory as a dataframe,  in a duckdb database file, in a netcdf4 file, or in a partitioned parquet file.  For the duckdb, netcdf4 and parquet options, 'aggregate_file' lets you define where to write the file.  If 'aggregate_file'  is not defined a temporary file is created following proper [R]{style="color:red"} guidelines, and in both cases the path to the file is returned.
...
does the reverse,  given an `rerddap::griddap()` extract with projected coordinates, the coordinates are translated into latitude and longitude values.
...
"fmt" defines how the split extracts are combined to give one result.  If fmt = "memory"  then the results are combined into an usual `rerddap::griddap()` dataframe in memory,  and that structure is returned by the function.  If fmt = "nc" then the extracts are combined into a netcdf4 file,  and a path to the netcdf4 file is returned by the function. If fmt = "duckdb" then the extracts are combined into a duckdb database file,  and a path to the duckdb file is returned by the function. If fmt = "parquet" then a directory is created and the parquet file for each split request is placed in the directory,  which is viewed as a partitioned parquet file.

Many [R]{style="color:red"} parquet readers, unlike in Python,  can not read a partitioned parquet file. To do so in [R]{style="color:red"}:

```{r, eval = FALSE, echo = TRUE}
df <- arrow::open_dataset("partitioned_parquet_files")
```

which does lazy access to the contents of the folder.  To actually obtain the data an action must be initiated, such as the following  read the entire download into memory:
...
or make a subset and then read into memory:

```{r, eval = FALSE, echo = TRUE}
df <- arrow::open_dataset("partitioned_parquet_files") |>
    dplyr::arrange(part_id, row_id) |>
    dplyr::collect()
```
...
The default "duckdb" format is used because it allows for incremental additions to the dataset on disk,  so datasets can be larger than memory.  To copy the duckdb file to parquet file if desired:
...
Similarly,  given an extract on a projected coordinate system, the values on terms of latitude and longitude can be found by:
