...
```
 ...
 The [R]{style="color:red"} package `rerddapUtils` is a set of four functions designed to work extend the `rerddap` package to provide capabilities requested by users, and that meet specialized needs which are better dealt with in a separate package.
 
 The first function is:
 
- `griddap_season <- function(datasetx, ..., fields = 'all', stride = 1, season = NULL, fmt = "nc", url = eurl(), store = disk(), read = TRUE, callopts = list())`
                          
which will extract data only for the period during the year defined by 'season'.  
...
which splits a very large request into pieces defined by 'request_split',  and then aggregates the parts either in memory,  in a duckdb database, or in a netcdf4 file.  This allows for requests larger than 2GB to be made in an appropriate manner.

In order 
 
Note that `griddap_season()` and `griddap_split()` are designed to mostly have the same interface as `rerddao::griddap()`,  except for one or two  extra function arguments,  as well as  some arguments interpreted differently, such as in both functions the argument 'store = disk()' is ignored.  In `griddap_season()` the 'fmt' argument is ignored  while 'season' defines the days during the year to make the extract.
...
Two helper functions are provided to work with `griddap_split()`:
...
`estimate_griddap_size()` provides an rough estimate of the size of the size of a download form an `rerddap::griddap()` request,  and `estimate_griddap_split_size()` takes the output from '`estimate_griddap_size()` and estimate the size of the download for each request for a given split of the original request.
...
does the reverse,  given an `rerddao::griddap()` extract with projected coordinates, the coordinates are translated into latitiude and longitude values.
...
it is desired to restrict the extract to March 1 to May 1 inclusive.  Then the following modification of the above using `griddap_season()`produces the desired result:
...
The argument "aggregate_file" is not set in the example above, so the netcdf file is written to a temporary location and the path to that file returned by the function.  The reason for this is it is bad form for an R package to write to a user's space without permission (in fact it is prohibited by CRAN).  To set the location where the netcdf file is written,  set "aggregate_file" to the name of the file,  in which case the file is written in the present working,  or else set "aggregate_file" as the full path to the file.  As an example:
...
To access the data in the netcdf file, any of the various R packages that can read netcdf4 files can be used, but it is recommend to only use packages that can read in subsets of the file,  as the data may well be too large for memory.  For example, to open the the netcdf file created above using 'ncdf4' and viewing a summary:
